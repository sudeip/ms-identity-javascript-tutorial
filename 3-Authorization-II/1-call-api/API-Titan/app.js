...
const bearerStrategy = new passportAzureAd.BearerStrategy({
    ...
    // Tokens for this API's exposed scope (api://<clientID>/access_as_user) carry
    // aud = "api://<clientID>" (the full Application ID URI), not the bare client ID.
    // Accept both forms since passport-azure-ad does a strict string match.
    audience: [authConfig.credentials.clientID, `api://${authConfig.credentials.clientID}`],
...
// Default changed from 6000 -> 6060: 6000 is on Chrome/Firefox's blocked "unsafe ports" list (old X11 port).
