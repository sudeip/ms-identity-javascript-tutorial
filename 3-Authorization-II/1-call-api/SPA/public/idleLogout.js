 ...
 *
 * Total idle timeout is IDLE_WARNING_MS + IDLE_LOGOUT_GRACE_MS = 1 minute
 * (30s before the warning appears, then a 30s countdown to sign-out) — still
 * short enough to demo live without a long wait, but closer to a real value
 * than the original 25s. For an actual deployment, tune both to whatever
 * balances a typical task's length against how long you're willing to leave
 * a session exposed.
 */

const IDLE_WARNING_MS = 30000; // inactivity before showing the warning
const IDLE_LOGOUT_GRACE_MS = 30000; // time the warning stays up before auto sign-out
